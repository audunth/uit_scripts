...
def condavg_2d(ts_1, ts_2, ts_avg, edges_ts1, edges_ts2):
    ...
    for t in ts1_idx_rg:
        ts1_idx_list.append(set(np.argwhere((ts_1 > edges_ts1[t]) & (ts_1 < edges_ts1[t + 1])).flatten()))
    for t in ts2_idx_rg:
        # Get ne indices where fluctuations are in between current edges
        ts2_idx_list.append(set(np.argwhere((ts_2 > edges_ts2[t]) & (ts_2 < edges_ts2[t + 1])).flatten()))
