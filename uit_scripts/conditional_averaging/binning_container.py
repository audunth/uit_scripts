...
class binning_container(object):
    ...
    def bin(self, array, feval_array=None):
        ...
        if (rv < self.bin_min):
            ...
            raise ValueError(
                'Could not bin array: %f < %f' %
                (rv, self.bin_min))

        idx = np.argwhere(np.array([(rv > t1) & (rv <= t2) for t1, t2 in
                                    self.bin_edges]))[0][0]
    ...
    def condvar(self, idx):
        ...
        tmp = (all_bursts -
               ca[np.newaxis, :].repeat(all_bursts.shape[0], axis=0))
        cvar = 1.0 - (tmp ** 2.0).mean(axis=0) / \
            (all_bursts ** 2.0).mean(axis=0)
        return(cvar)
...
if __name__ is "__main__()":
    ...
    for b_idx, b_tidx in enumerate(ref_burst_idx):
        ...
        # signal
        ts_ref_cut = ts_norm[b_tidx - burst_length:
                             b_tidx + burst_length]
        ...
        ts_x_cut = ts_x[b_tidx - burst_length:
                        b_tidx + burst_length]
