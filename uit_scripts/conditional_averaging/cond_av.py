def cond_av(S, T, smin, smax=None, Sref=None, delta=None, window=False):
    ...
    if window:
        ...
        while index < len(gpl_array):
            t_to_close = np.where(
                abs(gpl_array[
                    index+1:]-gpl_array[index]) < int(delta/dt))[0] + (index+1)
    ...
    for i in range(len(lT)):
        ...
        if high_ind == len(S):
            tmp_sn = np.append(tmp_sn,
                               np.zeros(global_peak_loc+t_half_len+1-len(S)))
    ...
    if lplcount > 0:
        print('There were problems locating unique peaks in {0} bursts, {1:.3f} of all bursts'.format(lplcount, lplcount/len(lT)))
        print('Largest number of peaks in burst:{0}\nLargest difference (data points) between peaks:{1}'.format(lplmax, lpldiff))
